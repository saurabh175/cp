...
int f(int si, int sl, int sr, int ql, int qr, int v)
{
    ...
    tre[si] = max(tre[2 * si], tre[2 * si + 1]);
    return maxs;
}
int ind = 0;
void search_ind(int si, int sl, int sr, int val)
{
    if (tre[si] < val)
    {
        ind = 0;
    ...
    if (tre[2 * si] >= val)
    {
        search_ind(2 * si, sl, mid, val);
    }
    else
    {
        search_ind(2 * si + 1, mid + 1, sr, val);
    }
}
void solve()
{
    ...
    while (q--)
    {
        int a;
        cin >> a;
        search_ind(1, 0, n - 1, a);
        cout << ind << " ";
        if (ind != 0)
            int x = f(1, 0, n - 1, ind-1, ind-1, tre[n+ind-1]-a);
    }
}
int32_t main()
{
